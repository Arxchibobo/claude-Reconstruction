...
timeout 3 python -m skill_seekers.mcp.server_fastmcp 2>/dev/null || {
    if [ $? -eq 124 ]; then
...
if python -c "import uvicorn" 2>/dev/null; then
    ...
    python -m skill_seekers.mcp.server_fastmcp --http --port 8765 > /dev/null 2>&1 &
    HTTP_TEST_PID=$!
    sleep 2
    ...
    if [ "$NEED_HTTP_SERVER" = true ]; then
        ...
        case $REPLY in
            1)
                ...
                nohup python -m skill_seekers.mcp.server_fastmcp --http --port $HTTP_PORT > /tmp/skill-seekers-mcp.log 2>&1 &
                SERVER_PID=$!
            ...
            2)
                ...
                echo ""
                echo -e "${GREEN}python -m skill_seekers.mcp.server_fastmcp --http --port $HTTP_PORT${NC}"
                echo ""
                echo "Or run in background:"
                echo -e "${GREEN}nohup python -m skill_seekers.mcp.server_fastmcp --http --port $HTTP_PORT > /tmp/skill-seekers-mcp.log 2>&1 &${NC}"
                ;;
            3)
...
if [ ${#SELECTED_AGENTS[@]} -gt 0 ]; then
    ...
    if [ "$HTTP_AVAILABLE" = true ]; then
        ...
        echo "   ${GREEN}python -m skill_seekers.mcp.server_fastmcp --http --port 3000${NC}"
        echo ""
...
echo "    ${CYAN}python -m skill_seekers.mcp.server_fastmcp${NC}"
echo ""
echo "  • Test HTTP server:"
echo "    ${CYAN}python -m skill_seekers.mcp.server_fastmcp --http${NC}"
